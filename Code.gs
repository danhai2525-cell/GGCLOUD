...
// ===== 🔐 MẬT KHẨU (ĐỔI NGAY 3 DÒNG NÀY TRƯỚC KHI DEPLOY) =====
// admin & tho01: dùng đăng nhập CRM. CTV_PASSWORD: dùng đăng nhập Ví CTV trên landing.
// Sau khi đổi ADMIN/KTV password, phải chạy lại resetPasswords() để cập nhật vào sheet.
var ADMIN_PASSWORD = 'GgC@Adm-2026x';   // ⚠️ ĐỔI mật khẩu admin
var KTV_PASSWORD   = 'GgC@Tho-2026x';   // ⚠️ ĐỔI mật khẩu thợ/KTV
var CTV_PASSWORD   = 'GgCtv@2026x';     // ⚠️ ĐỔI mật khẩu chung Ví CTV (phải khớp với frontend)
...
// ================== ĐỔI MẬT KHẨU (chạy sau khi sửa ADMIN_PASSWORD/KTV_PASSWORD) ==================
// Cập nhật mật khẩu admin & tho01 vào sheet mà KHÔNG xóa dữ liệu.
function resetPasswords() {
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('sheet_users');
  var data = sh.getDataRange().getValues();
  var updated = 0;
  for (var i = 1; i < data.length; i++) {
    var u = String(data[i][1]);
    if (u === 'admin') { sh.getRange(i + 1, 3).setValue(sha256_(ADMIN_PASSWORD)); updated++; }
    if (u === 'tho01') { sh.getRange(i + 1, 3).setValue(sha256_(KTV_PASSWORD)); updated++; }
  }
  return 'Đã đổi mật khẩu cho ' + updated + ' tài khoản.';
}
...
function setupDatabase() {
  ...
  // Tài khoản mẫu — mật khẩu lấy từ ADMIN_PASSWORD / KTV_PASSWORD ở đầu file (ĐỔI trước khi deploy!)
  var users = ss.getSheetByName('sheet_users');
  if (users.getLastRow() === 1) {
    users.appendRow(['TKAdmin01', 'admin', sha256_(ADMIN_PASSWORD), 'Danh', 'ADMIN', 'Hoạt động']);
    users.appendRow(['TKTho01', 'tho01', sha256_(KTV_PASSWORD), 'Hương', 'KTV', 'Hoạt động']);
  }
...
function route_(e, body) {
  ...
  var out;
  try {
    // ===== CHỐNG SPAM / FLOOD: giới hạn tần suất theo action nhạy cảm =====
    var guard = rateGuard_(e, action);
    if (!guard.ok) return jsonOut_({ success: false, error: guard.error });
    switch (action) {
...
// ===== RATE LIMITER (chống spam/DoS bằng CacheService) =====
// Giới hạn: mặc định 30 request / 60s / IP; các action ghi dữ liệu chặt hơn.
function rateGuard_(e, action) {
  try {
    var cache = CacheService.getScriptCache();
    var ip = (e && e.parameter && e.parameter.__ip) || 'anon';
    // Chặn payload rác quá lớn
    if (e && e.postData && e.postData.contents && e.postData.contents.length > 200000) {
      return { ok: false, error: 'Dữ liệu gửi lên quá lớn.' };
    }
    var LIMITS = {
      createOrder:     { max: 5,  win: 60 },
      requestWithdraw: { max: 3,  win: 60 },
      registerCTV:     { max: 5,  win: 60 },
      loginCTV:        { max: 10, win: 60 },
      login:           { max: 10, win: 60 },
      completeOrder:   { max: 5,  win: 60 }
    };
    var cfg = LIMITS[action] || { max: 40, win: 60 };
    var key = 'rl_' + action + '_' + ip;
    var cur = parseInt(cache.get(key) || '0', 10);
    if (cur >= cfg.max) {
      return { ok: false, error: 'Bạn thao tác quá nhanh. Vui lòng thử lại sau ít giây.' };
    }
    cache.put(key, String(cur + 1), cfg.win);
    return { ok: true };
  } catch (err) {
    return { ok: true }; // không để lỗi cache chặn dịch vụ
  }
}
...
function loginCTV(p) {
  ...
  if (!gmail || !/^\S+@\S+\.\S+$/.test(gmail)) return { success: false, error: 'Gmail không hợp lệ' };
  if (pass !== CTV_PASSWORD) return { success: false, error: 'Mật khẩu CTV không đúng.' };
  ...
  for (var i = orders.length - 1; i >= 1; i--) {
    if (String(orders[i][10]).toUpperCase() !== ctv.ctv_code) continue;
    refCount++;
    if (custRows.length < 100) {
      custRows.push({
        gmail: maskEmail_(orders[i][1]),
        package_type: orders[i][3],
        amount: Number(orders[i][6] || 0),
        status: orders[i][7],
        commission_ctv: Number(orders[i][11] || 0),
        created_at: String(orders[i][13])
      });
    }
  }
  ...
  return {
    ...
    customers: custRows, withdrawals: wl.withdrawals || []
  };
}

// Ẩn bớt email để bảo mật khách hàng: abc***@gmail.com
function maskEmail_(email) {
  var e = String(email || '');
  var at = e.indexOf('@');
  if (at < 1) return e;
  var name = e.slice(0, at), dom = e.slice(at);
  var keep = name.slice(0, Math.min(3, name.length));
  return keep + '***' + dom;
}
